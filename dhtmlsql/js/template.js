...
$(document).ready(function() {
	...
	//$('.element:parent').show(); $('.element h1:not(:contains('+$('#search-query').val()+'))').parent().hide();return false;
	
	//performs a search by scrolling to the target id	
	$('#search-query').on('keyup keypress change input',function(){
         var st = $(this).val();
		 f1='#method_'+st;
		 f2='#property_'+st;
		 if($(f1).length) {
		 $('a[href='+f1+']').trigger('click');
		 } else if($(f2).length) {
		 $('a[href='+f2+']').trigger('click');
		 }
		 
		 $(this).focus();
		 
		 //$('.element:parent').show(); $('.element h1:not(:contains('+$('#search-query').val()+'))').parent().hide();
    });
